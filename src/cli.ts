...
function parseArguments(arguments_: readonly string[]): CliOptions {
  if (arguments_[0] !== "configure-composer") {
    throw new Error("Usage: skir-laravel-data-generator configure-composer [--root <dir>] [--mod <module>]");
  }
  ...
  for (let index = 1; index < arguments_.length; index += 1) {
    ...
    if (argument !== "--root" && argument !== "--mod") {
      throw new Error(`Unknown argument ${argument}. Usage: skir-laravel-data-generator configure-composer [--root <dir>] [--mod <module>]`);
    }
